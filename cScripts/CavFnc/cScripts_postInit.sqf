...
if (EGVAR(Settings,enableStartHint)) then {
    { [EGVAR(Settings,setStartupDelay), EGVAR(Settings,setMissionType), EGVAR(Settings,setCustomHintTopic), EGVAR(Settings,setCustomHintText)] call FUNC(initStartupHint) } call CBA_fnc_directCall;
};

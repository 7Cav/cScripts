...
#ifdef DEBUG_MODE
    [format["Vehicle rack initzialized for %1", _vehicle], "Vehicle Radio" ] call FUNC(info);
#endif

private _racks = [_vehicle] call acre_api_fnc_getVehicleRacks;
if (count _racks == 0) exitWith {[format["No Vehicle Racks discoverd for %1.", _vehicle], "Vehicle Radio" ] call FUNC(info)};
...

{
    ...
    if (count _radio != 0) then {
        ...
        #ifdef DEBUG_MODE
            [format["Vehicle %1 radio %2 in rack %3 have radio set to channel %4", _vehicle, _radio, _x, _channel], "Vehicle Radio" ] call FUNC(info);
        #endif
    };
} forEach _racks;

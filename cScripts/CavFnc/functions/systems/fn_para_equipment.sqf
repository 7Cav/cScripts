...
switch (EGVAR(Settings,jumpSimulation)) do {
    case (1): { // Basic
        ...
        if (EGVAR(Settings,jumpSimulationNVG) && ((hmd _player != "") && (_nvgRandom > 4))) then {
            ...
            #ifdef DEBUG_MODE
                [format["Jump simulation %1 lost NVG (%2>4)", _player, _nvgRandom]] call FUNC(log);
            #endif
        };
        ...
        if (EGVAR(Settings,jumpSimulationHat) && ((headgear _player in _blacklist_headgear) && (_hatRandom > 3))) then {
            ...
            #ifdef DEBUG_MODE
                [format["Jump simulation %1 lost Hat (%2>32)", _player, _hatRandom]] call FUNC(log);
            #endif
        };
        ...
        if (EGVAR(Settings,jumpSimulationGlasses) && ((goggles _player in _blacklist_glasses) && (_glaRandom > 2))) then {
            ...
            #ifdef DEBUG_MODE
                [format["Jump simulation %1 lost Glasses (%2>2)", _player, _glaRandom]] call FUNC(log);
            #endif
        };
    };
    case (2): { // Advanced
        ...
        if (EGVAR(Settings,jumpSimulationHat) && ((headgear _player in _blacklist_headgear) && (_hatRandom > 3))) then {
            ...
            #ifdef DEBUG_MODE
                [format["Jump simulation %1 lost Hat (%2>3)", _player, _hatRandom]] call FUNC(log);
            #endif
        };

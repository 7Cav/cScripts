...
[{[] call acre_api_fnc_isInitialized}, {
    ...
    private _playerRadios = [(_this select 0)] call acre_api_fnc_getCurrentRadioList;
    {
        if !(_x == "") then {
            ...
            #ifdef DEBUG_MODE
                [format["%1 radio (%2) have its channel set to %3", (_this select 0), _x, _channel], "Radio"] call FUNC(info);
            #endif
            ...
            [format["Empty radio is trying to get it's channel applied for %1", (_this select 0)], "Radio"] call FUNC(warning);
        };

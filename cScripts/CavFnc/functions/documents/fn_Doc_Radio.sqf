...
_channelsSr = _channelsSr apply {
    ...
    _channel = [_channel] call FUNC(getChannelName);
    private _name = _x; 
    _x = if !(_squadName == "") then {
        format ["    <font color='#ffc61a'>%1</font> - %2", _squadName, _channel]; 
    } else {
        format [""]; 
    };
    _x
...
player createDiaryRecord ["7Cav", ["Radio", format ["<font size=20>%1</font><br/>%2<br/><br/><font size=20>%3</font><br/>%4", "<br/>Radio Channels", call (player getVariable QEGVAR(document,shortrange_channels)), "Long Range Radio", call (player getVariable QEGVAR(document,longrange_channels))]], taskNull, "", false];

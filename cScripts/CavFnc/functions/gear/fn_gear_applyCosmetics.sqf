...
if (EGVAR(Settings,allowInsigniaApplication)) then {
    ...
    if !(isNil {profileNamespace getVariable QEGVAR(Cav,Insignia)}) then {
        ...
        #ifdef DEBUG_MODE
            [format["%1 insignia '%2' obtained based on saved variable...", _unit, _insignia], "Gear"] call FUNC(info);
        #endif
    } else {
        _insignia = [_unit] call FUNC(getSquadInsignia);
        #ifdef DEBUG_MODE
            [format["%1 insignia '%2' obtained based on squad name...", _unit, _insignia], "Gear"] call FUNC(info);
        #endif
    };
    ...
    [{
        ...
        #ifdef DEBUG_MODE
            [format["%1 got insignia '%2' assinged", _this select 0, _this select 1], "Gear"] call FUNC(info);
        #endif

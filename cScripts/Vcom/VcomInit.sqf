...
if (isServer) then
{
    ...
    {
        private _Filecheck = loadFile "cScripts\userconfig\VCOM_AI\AISettingsV3.hpp";

        if !(_FileCheck isEqualTo "") then
        {
            [] call compile preprocessFileLineNumbers "cScripts\userconfig\VCOM_AI\AISettingsV3.hpp";
            [Vcm_Settings] remoteExec ["VCM_PublicScript",0,false];
        }
        else
        {
            [] call compile preprocessFileLineNumbers "cScripts\Vcom\Functions\VCOMAI_DefaultSettings.sqf";
            [Vcm_Settings] remoteExec ["VCM_PublicScript",0,false];
        };
    }
    else

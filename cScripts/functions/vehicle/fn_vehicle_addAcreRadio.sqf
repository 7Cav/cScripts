 ...
 *
 * Example:
 * ["vic"] call cScripts_fnc_vehicle_addAcreRadio;
 */
...
if (_vehicle iskindOf "MRAP_01_base_F") then {
    switch (_vehicleType) do {
        ...
        case "MED": {
            [_vehicle, [6]] call EFUNC(vehicle,setAcreRadio);
        };
        default {
            [_vehicle, [4]] call EFUNC(vehicle,setAcreRadio);
        };
    };
};
...
if (_vehicle iskindOf "rhsusf_stryker_base") then {
    switch (_vehicleType) do {
        ...
        default {
            [_vehicle, [3]] call EFUNC(vehicle,setAcreRadio);
        };
    };
};

if (_vehicle iskindOf "RHS_M2A2_Base") then {
    [_vehicle, [5]] call EFUNC(vehicle,setAcreRadio);
};
...
if (_vehicle iskindOf "RHS_UH60_Base") then {
    [_vehicle, [2, 9]] call EFUNC(vehicle,setAcreRadio);
};
...
if (_vehicle iskindOf "RHS_AH64_base") then {
    [_vehicle, [8]] call EFUNC(vehicle,setAcreRadio);
};
...
if (_vehicle iskindOf "Plane_Fighter_03_base_F") then {
    [_vehicle, [2, 8]] call EFUNC(vehicle,setAcreRadio);
};
